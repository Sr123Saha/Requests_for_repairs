...
def create_database(db_name='climate_repair.db'):
    ...
    CREATE TABLE requests (
        ...
        -- Поля для менеджера по качеству
    ...
    BEGIN
        ...
        VALUES (
            1, -- ID менеджера (нужно настроить)
...
def test_database_connection(db_name='climate_repair.db'):
    """
    Тестирует подключение к базе данных и показывает статистику
    """
    try:
        conn = sqlite3.connect(db_name)
        cursor = conn.cursor()
...
def export_schema_to_file(db_name='climate_repair.db', output_file='database_schema.sql'):
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"-- ============================================\n")
            f.write(f"-- СХЕМА БАЗЫ ДАННЫХ: climate_repair.db\n")
